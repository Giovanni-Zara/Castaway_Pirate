...
export class Environment {
    ...
    createSea() {
        ...
        position.original = position.array.slice();     //saving vertexes in array x0y0z0 x1y1z1... in order to save original position and not let animation accumulate
    ...
    updateSea(time) {
        ...
        for (let i = 0; i < position.count; i++) {
            ...
            position.array[ix + 2] = finalWave;     //new z position in the rotated plane-> height
        }
        position.needsUpdate = true;    //check if delete, never actually use it
        this.sea.geometry.computeVertexNormals();   //so lighting doesn't bug on waves after the deformations of the vertexes
    ...
    async createPlatforms() {   //ship is {x:150, y: -1.5, z: 0}        //partendo da z:-9 e x:70 fare i legni che affondano   
        ...
        rockPositions.forEach((pos, index) => {
            ...
            let rock;
            if (pos.x === 140 && pos.z === 6) {   //make this one yellow
                const rockMaterialYELLOW = new THREE.MeshStandardMaterial({ color: 0xFFFF00 });
                rock = new THREE.Mesh(rockGeometry, rockMaterialYELLOW);
            } else {
    ...
    createBubbleEffect(position) {
        for (let i = 0; i < 3; i++) {
            ...
            const bubbleGeometry = new THREE.SphereGeometry(0.08 + Math.random() * 0.04, 6, 4); // Vary size slightly,  (radius, horiz segment, vert segment)
            ...
            bubble.position.set(    //near the log but randomizing an offset to avoid too much overlapping
